...
export const ObjectText = (): JSX.Element => {
  ...
  const lines = useMemo(() => {
    const maxLines = Math.floor(constants.BASE_HEIGHT / (fontSize * LINE_HEIGHT));
    return chunkTextByWidth(constants.PREVIEW_TEXT, fontSize, constants.BASE_WIDTH + 5, maxLines);
  }, [ fontSize ]);
  ...
  return (
    <ObjectTextContainer ref={textRef} data-size={textSize}>
      { lines.map((line, index) => (
        ...
        >
          <text x="0" y={fontSize * (LINE_HEIGHT / 2)}>{ line }</text>
        </ObjectTextSvg>
      )) }
    </ObjectTextContainer>
  );
